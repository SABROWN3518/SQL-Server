...
-- Name: Steve Brown
-- Class: IT-112
-- Abstract: Review of IT-111 Audit tables and triggers
-- --------------------------------------------------------------------------------
...
IF OBJECT_ID('Z_TTeamPlayers') IS NOT NULL DROP TABLE Z_TTeamPlayers    -- Step 3 add drop table for audit table
IF OBJECT_ID('TPlayers') IS NOT NULL DROP TABLE TPlayers
IF OBJECT_ID('Z_TPlayers') IS NOT NULL DROP TABLE Z_TPlayers            -- Step 3 add drop table for audit table
...
(
	...
	,intPlayerID		INTEGER				NOT NULL
	,strModified_Reason	VARCHAR(1000)    -- Step 1 add reason column
	,CONSTRAINT TPlayerTeam_UQ UNIQUE ( intTeamID, intPlayerID )
...
(
	...
	,strAction				VARCHAR(1)			NOT NULL
	,strModified_Reason		VARCHAR(1000)   -- Step 1 add reason column
	,CONSTRAINT Z_PlayerTeam_UQ UNIQUE ( intTeamID, intPlayerID )
...
go
  -- step 4 add trigger TEAMS
CREATE TRIGGER tblTeamsTriggerAuditRecord on TTeams
...
go
  -- step 4 add trigger PLAYERS
CREATE TRIGGER tblPlayersTriggerAuditRecord on TPlayers
AFTER UPDATE, INSERT, DELETE 
AS
    ...
    SET @Now = GETDATE() -- Gets current date/time
			...
			IF (@Action='I')
				BEGIN --begin Insert info
					...
					FROM inserted I
						INNER JOIN TTeams T ON T.intTeamID = I.intPlayerID
...
go
  -- step 4 add trigger TEAMPLAYERS
CREATE TRIGGER tblTeamPlayerTriggerAuditRecord on TTeamPlayers
AFTER UPDATE, INSERT, DELETE 
AS
...
-- ------------------------------------------------------------------------------------------------------
-- Delete a record to test trigger and audit table entry                 (TeamPlayer)
-- ------------------------------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- Update a record to test trigger and audit table entry            
 --------------------------------------------------------------------------------
--UPDATE TTeamPlayers SET intTeamID = 2, strModified_Reason = 'TeamID changed'
--WHERE intTeamID = 3
...
-- ----------------------------------------------------------------------------------------------------------
-- Update a record to test trigger and audit table entry                  (Teams)
 ------------------------------------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- Delete a record to test trigger and audit table entry Teams
 --------------------------------------------------------------------------------
--SELECT * FROM TTEAMS
...
-- -----------------------------------------------------------------------------------------------------------
-- Update a record to test trigger and audit table entry                   (Player)
 -------------------------------------------------------------------------------------------------------------
--UPDATE TPlayers SET strFirstName = 'Bill', strModified_Reason = 'Player changed'
...
-- ------------------------------------------------------------------------------
-- Delete a record to test trigger and audit table entry
 --------------------------------------------------------------------------------
--SELECT * FROM TPlayers
